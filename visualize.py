...
def layout_lift(node: TreeNode, levels: int = 3) -> None:
    ...
    None
    """
    if int(node.Hd) == 1 and int(node.Ch) :
        n_name = node.index + ' ' + node.name
    elif int(node.Gap) == 1 or (float(node.u)>0 and not int(node.Ch)):
        n_name = node.index
    elif int(node.e) < levels:
        n_name = node.name
    else:
        n_name = ''

    # name = TextFace(node.name if (int(node.e) < levels or node.Hd == "1") else "", tight_text=True)
    name = TextFace(n_name, tight_text=True)
...
def draw_lifting_tree_inline(filename, save_pdf=False):
    ...
    # tree.show(tree_style=ts)
    if save_pdf:
        tree.render('.'.join(filename.split('.')[:-1]) + '.pdf', tree_style=ts)
    else:
        return tree.render("%%inline",tree_style=ts)
